...
//        return;
        ...
        [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            if (isDegraded) return;
            TOCropViewController *cropController = [[TOCropViewController alloc] initWithCroppingStyle:TOCropViewCroppingStyleDefault image:photo];
            //cropController.delegate = self;
            cropController.onDidCropToRect = ^(UIImage * _Nonnull image, CGRect cropRect, NSInteger angle) {
                [self.navigationController dismissViewControllerAnimated:YES completion:^{
                    [self callDelegateMethodWithPhotos:@[image] assets:nil infoArr:nil];
                }];
            };
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[TZImageManager manager] savePhotoWithImage:photo location:self.location completion:^(PHAsset *asset, NSError *error){
                if (!error) {
                    [self addPHAsset:asset imageInfo:photo];
                }
            }];
            self.location = nil;
        }
        ...
        if (videoUrl) {
            [[TZImageManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(PHAsset *asset, NSError *error) {
                if (!error) {
                    [self addPHAsset:asset imageInfo:nil];
                }
            }];
            self.location = nil;
        }
...
- (void)addPHAsset:(PHAsset *)asset imageInfo:(UIImage *)info {
...
#warning push crop
             ...
             */
            //UIImage *image = assetModel.asset
            TOCropViewController *cropController = [[TOCropViewController alloc] initWithCroppingStyle:TOCropViewCroppingStyleDefault image:info];
            //cropController.delegate = self;
            cropController.onDidCropToRect = ^(UIImage * _Nonnull image, CGRect cropRect, NSInteger angle) {
                [self.navigationController dismissViewControllerAnimated:YES completion:^{
                    [self callDelegateMethodWithPhotos:@[image] assets:nil infoArr:nil];
